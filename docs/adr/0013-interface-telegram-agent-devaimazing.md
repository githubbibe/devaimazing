...
- **Non fait par cet ADR lui-même — fait séparément le 2026-07-24, tranches
  S1 à S3 (voir `docs/roadmap.md`)** : le registre d'outils avec métadonnées
  de confirmation (`runtime/studio/tools/registry.py`), les handlers de
  lecture seule (`lire_statut`, `lire_progression`, `lister_projets`, dans
  `runtime/studio/tools/queries.py`) et d'écriture (`creer_projet`,
  `archive_projet` — `bot.create_forum_topic`/`close_forum_topic`,
  `commit_safety_snapshot`/`push_branch` avant archivage), l'écriture
  ciblée de `thread_id` dans `config/projects/<nom>.yml`
  (`runtime/studio/tools/project_config.py`), le bot Telegram lui-même
  (`runtime/studio/telegram/`, commande `devaimazing telegram-bot`,
